...
1. Depois que os arquivos forem carregados, expanda **Arquivos** e clique na pasta **orders**. Verifique se os arquivo CSV foram carregados, conforme mostrado aqui:
    ...
    ![Imagem da tela dos arquivos CSV carregados em um novo espaço de trabalho do Fabric.](Images/uploaded-files.png)
...
1. Use o botão 🖉 (Editar) para alternar a célula para o modo de edição e modifique o Markdown como mostrado abaixo.
   ...
   Use this notebook to explore sales order data
    ...
    ![Imagem de tela de um notebook Fabric com uma célula Markdown.](Images/name-notebook-markdown.png)
...
>[!NOTE]
> Os notebooks do Fabric aceitam várias linguagens de programação, incluindo Scala, R e Spark SQL.
...
1. Selecione o lakehouse para exibir o painel Explorer, incluindo a pasta **orders**.
1. No menu superior, clique em **Abrir notebook**, **Notebook existente** e, em seguida, abra o notebook criado anteriormente. O notebook abrirá ao lado do painel Explorer. Expanda Lakehouses, expanda a lista Arquivos e selecione a pasta "orders". Os arquivos CSV que você carregou são listados ao lado do editor de notebook, desta maneira:

    ![Imagem da tela de arquivos csv na exibição do Explorer.](Images/explorer-notebook-view.png)

1. No menu ... de 2019.csv, selecione **Carregar dados** > **Spark**. O código é gerado automaticamente em uma nova célula de código:
...
>[!TIP]
> Você pode ocultar os painéis do Explorador à esquerda usando os ícones «. Isso dá mais espaço para o notebook.

1. Clique em ▷ **Executar célula** à esquerda da célula para executar o código.

>[!NOTE]
> A primeira vez que você executar o código Spark, uma sessão do Spark será iniciada. Isso pode levar alguns segundos ou mais. As execuções subsequentes na mesma sessão serão mais rápidas.

1. Quando o código da célula for concluído, analise a saída abaixo da célula, que deve ser semelhante a:
 
    ![Imagem da tela mostrando código e dados gerados automaticamente.](Images/auto-generated-load.png)

1. A saída mostra os dados do arquivo 2019.csv exibidos em linhas e as colunas.  Perceba que os cabeçalhos das colunas contêm a primeira linha dos dados. Para corrigir isso, você precisa modificar a primeira linha do código da seguinte maneira:

    ```python
   df = spark.read.format("csv").option("header","false").load("Files/orders/2019.csv")
    ```

1. Execute o código novamente, para que o DataFrame identifique corretamente a primeira linha como dados. Perceba que os nomes das colunas agora mudaram para _c0, _c1, etc.

1. Nomes de coluna descritivos ajudam você a entender os dados. Para criar nomes de coluna significativos, você precisa definir o esquema e os tipos de dados. Você também precisa importar um conjunto padrão de tipos SQL do Spark para definir os tipos de dados. Substitua o código existente pelo seguinte:
...
1. Execute a célula e analise a saída:

    ![Imagem da tela do código com esquema definido e dados.](Images/define-schema.png)

1.  Esse DataFrame só inclui os dados do arquivo 2019.csv. Modifique o código para que o caminho do arquivo use um curinga * para ler todos os dados na pasta orders:
...
1.  Ao executar o código modificado, você verá as vendas de 2019, 2020 e 2021. Apenas um subconjunto das linhas é exibido, portanto, talvez você não veja as linhas de todos os anos.

>[!NOTE]
> Você pode ocultar ou exibir a saída de uma célula selecionando **…** ao lado do resultado. Isso facilita o trabalho em um notebook.
...
1.  O código a seguir filtrará os dados para que apenas duas colunas sejam retornadas. Ele também usa *count* e *distinct* para resumir o número de registros:
...
1. Execute o código e analise a saída:
...
1. Modifique a primeira linha do código usando *select* com uma função *where* da seguinte maneira:
...
1. Execute o código modificado para selecionar apenas os clientes que compraram o produto Road-250 Red, 52. Você pode "encadear" várias funções para que a saída de uma função se torne a entrada da próxima. Nesse caso, o DataFrame criado pelo método *select* é o DataFrame de origem do método **where** usado para aplicar critérios de filtragem.
...
1. Execute o código. Observe que os resultados mostram a soma das quantidades de pedido agrupadas por produto. O método *groupBy* agrupa as linhas por Item, e a função de agregação *sum* subsequente é aplicada às colunas numéricas restantes, nesse caso, *Quantidade*.

1. Adicione outra célula de código ao notebook e insira o seguinte código nela:
...
1. Execute a célula. Examine o resultado. Os resultados mostram o número de pedidos de vendas por ano:
    ...
    * A contagem de linhas em cada grupo é calculada antes de o método *orderBy* ser usado para classificar o DataFrame resultante.
    ...
    ![Imagem de tela mostrando os resultados da agregação e agrupamento de dados em um DataFrame.](Images/spark-sql-dataframe.png)
...
1. Execute a célula. Um novo DataFrame com base nos dados do pedido original com as seguintes transformações:
...
1. Analise a saída e verifique se as transformações foram feitas nos dados.
...
1. Execute a célula e aguarde a mensagem indicando que os dados foram salvos. Em seguida, no painel do Explorer à esquerda, no menu ... do nó Arquivos, clique em **Atualizar**. Escolha a pasta transformed_data para verificar se ela contém uma nova pasta chamada orders, que por sua vez contém um ou mais arquivos Parquet.

1. Adicione uma célula com o seguinte código:
...
1. Execute a célula.  Um novo DataFrame é criado a partir dos arquivos parquet na pasta *transformed_data/orders*. Verifique se os resultados mostram os dados do pedido que foram carregados a partir dos arquivos Parquet.

    ![Imagem da tela mostrando arquivos Parquet.](Images/parquet-files.png)
...
1.  Execute a célula e aguarde a mensagem indicando que os dados foram salvos. Em seguida, no painel do Lakehouses à esquerda, no menu ... do nó Arquivos, clique em **Atualizar** e expanda a pasta partitioned_data para verificar se ela contém uma hierarquia de pastas chamada *Year=xxxx*, cada uma contendo pastas chamadas *Month=xxxx*. Cada pasta mensal contém um arquivo Parquet com os pedidos desse mês.

    ![Imagem da tela mostrando dados particionados por Year e Month.](Images/partitioned-data.png)

1. Adicione uma nova célula com o seguinte código para carregar um novo DataFrame a partir do arquivo orders.parquet:
...
1. Execute a célula e verifique se os resultados mostram os dados do pedido de vendas em 2021. Perceba que as colunas de particionamento especificadas no caminho (Year e Month) não estão incluídas no DataFrame.
...
1. Execute a célula de código e analise a saída, que descreve a definição da nova tabela.

1. No painel do **Explorador**, no menu ... da pasta Tabelas, clique em **Atualizar**. Em seguida, expanda o nó **Tabelas** e verifique se a tabela **salesorders** foi criada.

    ![Imagem da tela mostrando que a tabela salesorders foi criada.](Images/salesorder-table.png)

1. No menu … da tabela salesorders, clique em **Carregar dados** > **Spark**. Uma nova célula de código que contém um código semelhante ao seguinte exemplo é adicionada:
...
1. Execute o novo código, que usa a biblioteca do Spark SQL para inserir uma consulta SQL na tabela *salesorder* no código PySpark e carregar os resultados da consulta em um DataFrame.
...
1. Execute a célula e analise os resultados. Observe que:
...
1. Execute o código para exibir dados da exibição salesorders que você criou anteriormente. Na seção de resultados abaixo da célula, selecione **+ Novo gráfico**.

1.  Use o botão **Criar meu próprio** no canto inferior direito da seção de resultados e defina as configurações do gráfico:
    ...
    * Agregação: Soma
    * Valores ausentes e NULL: exibir como 0
    * Empilhado: Não selecionado

1. Seu gráfico será semelhante a este:

    ![Imagem da tela da visualização do gráfico do notebook do Fabric.](Images/built-in-chart.png) 
...
1. Execute o código. Ele retornará um DataFrame do Spark contendo a receita anual e o número de pedidos. Para visualizar os dados como um gráfico, começaremos usando a biblioteca matplotlib em Python. Essa biblioteca é a biblioteca de plotagem principal na qual muitas outras se baseiam e fornece muita flexibilidade na criação de gráficos.

1. Adicione uma nova célula de código e o seguinte código:
...
1. Execute a célula e analise os resultados, que consistem em um gráfico de colunas com a receita bruta total de cada ano. Examine o código e observe o seguinte:
...
1.  Modifique o código para plotar o gráfico da seguinte maneira:
...
1. Execute novamente a célula de código e veja os resultados. Agora está mais fácil de entender o gráfico.
1. Um gráfico está contido com uma Figura. Nos exemplos anteriores, a figura foi criada implicitamente, mas pode ser criada explicitamente. Modifique o código para plotar o gráfico da seguinte maneira:
...
1. Execute novamente a célula de código e veja os resultados. A figura determina a forma e o tamanho do gráfico.
1. Uma figura pode conter vários subgráficos, cada um em um eixo próprio. Modifique o código para plotar o gráfico da seguinte maneira:
...
1. Execute novamente a célula de código e veja os resultados. 
...
1. Execute o código para exibir um gráfico de barras usando a biblioteca seaborn.
1. Modifique o código da seguinte maneira:
...
1.  Execute o código modificado e observe que a seaborn permite que você defina um tema de cor para os gráficos.
1.  Modifique o código novamente da seguinte maneira:
...
1.  Execute o código modificado para ver a receita anual como um gráfico de linhas.
